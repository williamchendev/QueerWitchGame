...
if (ds_list_size(flash_timer) > 0) {
	...
	for (var f = ds_list_size(flash_timer) - 1; f >= 0; f--) {
		...
		// Draw Bullet Trail
		draw_set_alpha(0.3 * inverse_smooth_lerp(power((temp_flash_timer / flash_delay), 2)));
		...
		if (muzzle_flash_sprite != noone) {
			draw_sprite_ext(muzzle_flash_sprite, temp_flash_imageindex, temp_flash_xposition, temp_flash_yposition, inverse_smooth_lerp(power((temp_flash_timer / flash_delay), 2)), weapon_yscale, temp_flash_direction, c_white, 1);
		}
	}
	
	// Gun Muzzle Flash
	draw_sprite_ext(muzzle_flash_sprite, 0, temp_muzzle_x, temp_muzzle_y, inverse_smooth_lerp(power((temp_flash_timer / flash_delay), 2)), weapon_yscale, temp_weapon_rotation, c_white, 1);
}
