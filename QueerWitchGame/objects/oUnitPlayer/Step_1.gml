...
if (camera_follow) {
	...
	if (inventory_show) {
		// Inventory
		if (inventory != noone) {
			target_pos_x = x + ((inventory.inventory_width / 2) * inventory.inventory_grid_size) + inventory.inventory_offset_size;
			target_pos_y = y + ((((inventory.inventory_height / 2) * inventory.inventory_grid_size) + inventory.inventory_offset_size) * 0.6);
		}
	}
	/*
