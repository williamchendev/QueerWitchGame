...
if (draw_stats_mode == "health") {
	...
	draw_set_alpha(0.8 * draw_stats_alpha * draw_stats_alpha);
	draw_set_color(c_black);
	draw_text(temp_stats_x + 1, temp_stats_y - 16, format_str_int_comma(round(calorie_show)) + " Calories");
	
	draw_set_alpha(draw_stats_alpha * draw_stats_alpha);
	draw_set_color(c_white);
	draw_text(temp_stats_x, temp_stats_y - 17, format_str_int_comma(round(calorie_show)) + " Calories");
}
...
if (select_alpha > 0.05) {
	...
	if (gui_mode = "actions") {
		...
		for (var i = 0; i < select_list_length; i++) {
			...
			var temp_action_sin = ((i - select_menu_draw_pos) * 15) + 15;
			var temp_action_x = temp_select_x - 120 + lengthdir_x(140, temp_action_sin);
