/// raycast_ground_ignore_edge(x,y,raycast_length,edge);
/// @description Raycasts a line down from the given coordinate for as long as the raycast length to check for a platform or a solid while ignoring all platforms that intersect with the given edge
...
/// @param {real} edge The oPathEdge to Ignore
...
var temp_edge = argument3;

var temp_a_x = temp_edge.nodes[0].x;
var temp_a_y = temp_edge.nodes[0].y;

var temp_b_x = temp_edge.nodes[1].x;
var temp_b_y = temp_edge.nodes[1].y;
...
if (temp_platform_num > 0) {
    for (var i = 0; i < temp_platform_num; i++) {
		...
		if (!collision_line(temp_a_x, temp_a_y, temp_b_x, temp_b_y, temp_platform, false, true)) {
