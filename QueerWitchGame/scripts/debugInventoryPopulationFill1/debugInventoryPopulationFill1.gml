/// debugInventoryPopulationFill1(inventory_object);
/// @description Takes an inventory object and populates the inventory array data with random formatted indexes
/// @param {real} inventory_object The given inventory object to populate with indexes

var temp_inven_obj = argument0;

placeItemInventory(temp_inven_obj, 1, 0, 0);
placeItemInventory(temp_inven_obj, 1, 1, 0);
placeItemInventory(temp_inven_obj, 2, 2, 2, -1);
placeItemInventory(temp_inven_obj, 2, 1, 2);
placeItemInventory(temp_inven_obj, 3, 4, 2);